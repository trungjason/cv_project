...
$tastRate = isset($_POST['rate']) ? $_POST['rate'] : "";

if ($taskID == "") {
    die(json_encode(array('status' => false, 'errorMessage' => 'Không tìm thấy Mã nhiệm vụ cần đánh giá')));
}

if ($tastRate == "") {
    die(json_encode(array('status' => false, 'errorMessage' => 'Vui lòng chọn mức độ đánh giá')));
}
...
if ($checkTaskStatus['result']['Status'] != 'Waiting') {
    die(json_encode(array('status' => false, 'errorMessage' => 'Trưởng phòng không thể đánh giá Nhiệm vụ ở trạng thái ' . $checkTaskStatus['result']['Status'])));
}

die(json_encode(rateTask($taskID, $tastRate)));
